...
// Local type for user lookup display (grade tier info shown before login)
...
function useUserLookup() {
  const [userInfo, setUserInfo] = useState<LookupUser | null>(null);
  ...
  const lookupUser = async (email: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/auth/user-lookup`, {
        method: 'POST',
        headers: {
  ...
  const simulateUserLookup = (email: string) => {
    const mockUsers: Record<string, Partial<LookupUser>> = {
      'test@gmail.com': {
        id: '1',
        first_name: 'Test',
        last_name: 'Student',
    ...
    setUserInfo(userData as LookupUser || null);
  };
...
function SignInPage() {
  ...
  const { user, login, loading: loginLoading, error, clearError } = useAuth();
  const { userInfo, lookupUser, setUserInfo } = useUserLookup();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After login, useAuth will have the updated user — read grade_category from auth context
      // We show success and redirect; user state will be updated by the auth context
      setSuccessMessage(`Welcome back! Redirecting to your dashboard...`);

      // Redirect to dashboard router page which will redirect to the right tier
      setTimeout(() => {
        router.push('/dashboard');
      }, 2000);
            ...
            <CardContent className="space-y-6">
              ...
              {process.env.NODE_ENV === 'development' && (
                <div className="text-xs text-gray-500 bg-gray-50 p-2 rounded">
                  Backend: {process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}
                </div>
              )}
